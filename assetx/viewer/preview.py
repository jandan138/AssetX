...
class Previewer:
    ...
    def _preview_with_trimesh(
        self, mesh_path: Path, show_axes: bool, show_wireframe: bool
    ) -> None:
        """使用trimesh预览"""
        try:
            import trimesh
            ...
            logger.info(
                f"Vertices: {len(mesh.vertices) if hasattr(mesh, 'vertices') else 'N/A'}"
            )
            logger.info(
                f"Faces: {len(mesh.faces) if hasattr(mesh, 'faces') else 'N/A'}"
            )
    ...
    def preview_urdf(self, urdf_path: Union[str, Path]) -> None:
        ...
        try:
                        ...
                        if visual.geometry.mesh:
                            try:
                                ...
                                if mesh_path.exists():
                                    ...
                                    scene.add_geometry(
                                        mesh, node_name=f"{link.name}_{visual.name}"
                                    )

                            except Exception as e:
                                logger.warning(
                                    f"Failed to load mesh for {link.name}: {e}"
                                )
            ...
            raise ImportError(
                "urdfpy is required for URDF preview. Install with: pip install urdfpy"
            )
        except Exception as e:
    ...
    def _capture_with_open3d(
        self, mesh_path: Path, output_path: Path, resolution: tuple
    ) -> Path:
        """使用Open3D捕获截图"""
        try:
            import open3d as o3d
        ...
        try:
            if self.backend == "trimesh":
                ...
                stats = {
                    ...
                    "is_watertight": (
                        mesh.is_watertight if hasattr(mesh, "is_watertight") else False
                    ),
                ...
                stats = {
                    ...
                    "bounds": [
                        mesh.get_min_bound().tolist(),
                        mesh.get_max_bound().tolist(),
                    ],
