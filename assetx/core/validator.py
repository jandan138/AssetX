...
class PhysicsValidator:
    ...
    def compare_assets(self, asset1: Asset, asset2: Asset) -> ValidationResult:
        ...
        for link_name in common_links:
            if link_name in asset1.physics_properties and link_name in asset2.physics_properties:
    ...
    def _validate_masses(self, asset: Asset, result: ValidationResult) -> None:
        ...
        for link_name, props in asset.physics_properties.items():
            if props.mass is not None:
                if props.mass <= 0:
                    result.add_error(f"Invalid mass for link '{link_name}': {props.mass}")
                elif props.mass < 1e-6:
                    result.add_warning(f"Very small mass for link '{link_name}': {props.mass}")
    ...
    def _check_inertia_triangle_inequality(self, ixx: float, iyy: float, izz: float) -> bool:
        """检查惯性矩阵三角不等式"""
        return (ixx + iyy >= izz and 
                iyy + izz >= ixx and 
                izz + ixx >= iyy)
    
    def _validate_geometry_consistency(self, asset: Asset, result: ValidationResult) -> None:
        """验证几何一致性"""
        # TODO: 实现几何一致性检查
        # 例如：检查碰撞网格是否包含在视觉网格内
        pass
    ...
    def _compare_joints(self, asset1: Asset, asset2: Asset, result: ValidationResult) -> None:
        """比较关节信息"""
            ...
            if joint1.limits and joint2.limits:
                for limit_type in ['lower', 'upper', 'effort', 'velocity']:
                    if limit_type in joint1.limits and limit_type in joint2.limits:
                        diff = abs(joint1.limits[limit_type] - joint2.limits[limit_type])
