...
class MetaManager:
    ...
    def add_conversion_record(self, 
                            asset_name: str,
                            target_format: str,
                            output_path: str,
    ...
    def add_validation_record(self,
                            asset_name: str,
                            validation_type: str,
                            result: Dict,
                            compared_with: Optional[str] = None) -> None:
        """添加验证记录"""
    ...
    def export_meta_report(self, output_path: Optional[Union[str, Path]] = None) -> Path:
        """导出元数据报告"""
        meta_data = self.load_meta()
        ...
        for asset_name, asset_meta in meta_data.get('assets', {}).items():
            ...
            report['summary']['formats'][format_name] = report['summary']['formats'].get(format_name, 0) + 1
            ...
            if asset_meta.get('validations'):
                report['summary']['validation_stats']['total_validations'] += len(asset_meta['validations'])
