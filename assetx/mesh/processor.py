...
class MeshProcessor:
    ...
    def simplify_mesh(
        ...
        try:
            ...
            if original_faces <= target_faces:
                logger.info(
                    f"Mesh already has {original_faces} faces, less than target {target_faces}"
                )
                return mesh_path
            ...
            logger.info(
                f"Simplified mesh from {original_faces} to {len(simplified.faces)} faces"
            )
            return output_path
    ...
    def normalize_units(
        ...
        if source_unit not in unit_scales or target_unit not in unit_scales:
            raise ValueError(
                f"Unsupported units. Supported: {list(unit_scales.keys())}"
            )
    ...
    def get_mesh_info(self, mesh_path: Union[str, Path]) -> dict:
        ...
        try:
            ...
            info = {
                ...
                "center_mass": (
                    mesh.center_mass.tolist() if hasattr(mesh, "center_mass") else None
                ),
            }
