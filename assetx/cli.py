...
@click.option('--update-meta', is_flag=True, default=True,
...
@click.option('--backend', default='trimesh', 
...
@main.command()
@click.option('--ref', required=True, type=click.Path(exists=True),
...
@click.option('--target-faces', type=int, default=1000,
              help='Target face count for simplification')
@click.option('--scale-factor', type=float, default=1.0,
              help='Scale factor for scaling operation')
@click.option('--collision-method', default='convex_hull',
...
def mesh(path, operation, output, target_faces, scale_factor, collision_method):
    ...
    try:
        ...
        if operation == 'info':
            ...
            output_path = processor.generate_collision_mesh(mesh_path, collision_method, output)
...
@main.command()
@click.option('--dir', '-d', default='.', type=click.Path(exists=True),
    ...
    try:
        ...
        if export:
            ...
            for asset in assets:
                ...
                click.echo(f"    Physics Validated: {'✅' if asset.get('physics_validated') else '❌'}")
...
@click.option('--path', required=True, type=click.Path(exists=True), help='Asset file path')
@click.option('--category', required=True, help='Semantic category (e.g., robot_arm, mobile_robot, gripper)')
